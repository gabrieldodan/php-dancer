<?php
class PFC_RadioList extends PFC_List {
	/**
	 * 
	...
	static function newInst($name, $dataSet, $value = array()) {
		return new PFC_RadioList($name, $dataSet, $value);
	}
	
	function buildContent() {
		// create items
		foreach ($this->dataSet AS $k => $v) {
			$this->items[] = new PFCI_Radio($this, $k, $v, in_array($k, $this->value));
		}
